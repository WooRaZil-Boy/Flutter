...
class _NotificationDialogState extends DialogState<NumberDialog> {
  ...
  // 입력시 포커스 탐지를 위해서는 FocusNode가 필요하다.
  ...
  Widget build(BuildContext context) {
    // 키보드가 올라오면 화면이 올라가는 것을 방지하기 위해 Material을 사용한다.
    // 반대로 화면이 올라가야 한다면, Scaffold를 사용한다.
    // return Material(
          ...
          RoundedContainer(
            ...
            children: [
              ...
              TextField(
                ...
                obscureText: true, // 암호 입력 시, 가려주는 효과를 준다.
                controller:
                    passwordController, // 같은 controller를 사용하면, 입력한 값이 동일하게 나타나게 된다.
                keyboardType: TextInputType.number,
              ),
              RoundButton(
                text: '완료',
                onTap: () async {
                  ...
                  try {
                    int number = int.parse(text); // 숫자만 파싱하기 때문에 문자열을 사용하면 오류가 catch 된다.
